...
[ ] OWUI 自己登録を再開 (docker-compose.yml: ENABLE_SIGNUP=true)
    院外フェーズは HINES に公開中のため false (無差別な自己登録を遮断)。
    閉域内では「自己登録 → 管理者承認 (DEFAULT_USER_ROLE=pending)」で運用するため true に戻す。
    反映: docker compose up -d open-webui (ENABLE_PERSISTENT_CONFIG=false なので env が即反映)
```
