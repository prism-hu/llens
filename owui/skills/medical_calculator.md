...
| 腫瘍 | `calc_calvert_carboplatin` | カルボプラチン投与量 (Calvert式) |
...
### `calc_calvert_carboplatin`

Calvert式 (`dose = AUC × (GFR + 25)`) によるカルボプラチン投与量 (mg)。

- **GFR に推算CCr (Cockcroft-Gault) を使う場合、過量投与防止のため 125 mL/min で上限クリップ**される (`cap_gfr_at_125=True`、初期値)。実測GFR (EDTA/イオヘキソール) を使う場合は `False` にする
- GFR は **絶対値 (mL/min)** を渡す。eGFR (体表面積補正済み mL/min/1.73m²) ではなく、`calc_ccr_cockcroft_gault` の CCr を使うのが標準
- 目標AUCはレジメン規定に従う (単剤 5-7、併用 2-6 が目安)。本関数は式計算のみで、レジメン妥当性は判断しない
- オピオイドの用量換算は本tool群ではなく `opioid-conversion` SKILL (換算表ベース) を参照
...
- **`opioid-conversion`**: オピオイド等価換算 (経口モルヒネ基準の換算表)。換算比が施設・ガイドラインで揺れ、不完全交差耐性による減量判断を伴うため、tool 化せず換算表を引く reference SKILL として実装
