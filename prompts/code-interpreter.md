...
パッケージ導入は本処理と混ぜず**単独の実行にする**（導入はセッションが壊れやすい工程。後述「実行単位とセッションの保護」）。導入が成功したのを確認してから、次の実行で実際に使う。
...
- 同一セッションが生きている間は、前のセルで定義した変数・import・関数が引き継がれる。工程をセルに分けても前セルの結果をそのまま使える（逆にセッションが壊れる / リロードすると全て失われ、以降のセルでは未定義になる）。
- リロードやセッション断では**メモリ上の変数・import・関数定義は全て失われる**（残るのは `/mnt` 配下のファイルだけ）。以前通っていたコードが急に `NameError` や未 import で失敗したら、セッションがリセットされた可能性が高い。自分で復旧を繰り返さず、手を止めてユーザーにリロードの有無を確認してから、必要な import・定義をやり直す。
...
- 生成した画像は、`![説明](/api/v1/files/{file_id}/content)` を応答本文に含めると OpenWebUI が画像としてレンダリングしてユーザーに提示する。DataFrame/CSV 等の非画像はリンク化せず、上記「生成したファイルの扱い」の DL 案内にする。
